...
def get_database_config():
    ...
    return {
        ...
        if DEBUG
        else require_env("DB_NAME"),
        "USER": os.getenv("DB_USER", "postgres")
        if DEBUG
        else require_env("DB_USER"),
        "PASSWORD": os.getenv("DB_PASSWORD", "")
        if DEBUG
        else require_env("DB_PASSWORD"),
        "HOST": os.getenv("DB_HOST", "localhost")
        if DEBUG
        else require_env("DB_HOST"),
        "PORT": os.getenv("DB_PORT", "5432")
        if DEBUG
        else require_env("DB_PORT"),
    }
...
# Automatically attach Render's hostname if deployed on Render
...
if not DEBUG and not CORS_ALLOW_ALL_ORIGINS and not CORS_ALLOWED_ORIGINS:
    raise ValueError(
        "Set CORS_ALLOWED_ORIGINS or CORS_ALLOW_ALL_ORIGINS in production"
    )
